...
{
    public class YouYouJoystick2 : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
    {
        ...
        void IDragHandler.OnDrag(PointerEventData eventData)
        {
            ...
            Vector2 direction = eventData.position - (Vector2)backGround.position; //得到pointerDownPos 指向 OnDragPos 的向量
            ...
            if (dynamic)
            {
                if (Vector3.Distance(eventData.position, backGround.position) > maxRadius)
                {
                    Vector3 backGroundPos = eventData.position - localPosition;
                    backGround.position = eventData.pressEventCamera?.ScreenToWorldPoint(backGroundPos) ?? backGroundPos;
                }
            }
